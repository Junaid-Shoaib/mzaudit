        ...
        <table class="shadow-lg border mt-4 ml-12 rounded-xl w-11/12">
          ...
          <tbody>
            <tr v-for="item in balances.data" :key="item.id">
              <td Style="width: 25%" class="py-2 px-2 border text-left">
                {{ item.name }}
              </td>
              <td
                Style="width: 50%"
                class="py-2 px-2 border text-center text-transform: capitalize"
              >
                {{ item.address }}
              </td>
              <td Style="width: 25%" class="py-2 px-2 border text-center">
                <inertia-link
                  class="
                    border
                    bg-blue-400
  ...
  watch: {
    params: {
      handler: throttle(function () {
        ...
        this.$inertia.get(this.route("branches"), params, {
          replace: true,
          preserveState: true,
        });
      }, 150),
